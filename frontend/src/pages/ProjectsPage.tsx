...
import { Search, MapPin, CheckCircle, Activity, Filter, Globe } from 'lucide-react';
import { projectsData, MAQASID, AVAILABLE_SDGS, SDG_DETAILS } from '../data/mockData';
import { MaqasidIcon, SdgBadge } from '../components/ui/Badges';

const ProjectsPage = () => {
  const [selectedMaqasid, setSelectedMaqasid] = useState<string[]>(Object.keys(MAQASID));
  ...
  const filteredProjects = useMemo(() => {
    return projectsData.filter(project => {
      const hasMaqasid = project.maqasid.some(m => selectedMaqasid.includes(m));
    ...
    <div className="bg-gray-50 min-h-screen py-10">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-10 gap-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Verified Waqf Projects</h1>
            <p className="text-gray-500 mt-2">Invest in high-impact, Sharia-compliant projects vetted by AI.</p>
          </div>
          ...
          <div className="w-full lg:w-64 shrink-0 space-y-8">
             <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                ...
                <div className="space-y-3">
                    ...
                    <label key={key} className="flex items-center text-sm text-gray-600 hover:text-gray-900 cursor-pointer">
                        <input
                        type="checkbox"
             ...
             <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                ...
                <div className="space-y-3">
                    ...
                    <label key={sdg} className="flex items-center text-sm text-gray-600 hover:text-gray-900 cursor-pointer">
                        <input
                        type="checkbox"
                        className="mr-3 rounded text-green-600 focus:ring-green-500 cursor-pointer w-4 h-4 border-gray-300"
          ...
          <div className="flex-1">
            {filteredProjects.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 bg-white rounded-xl border border-dashed border-gray-300">
                    ...
                    <p className="text-gray-500 font-medium">No projects match the selected filters.</p>
                    <button
                        onClick={() => { setSelectedMaqasid(Object.keys(MAQASID)); setSelectedSdgs(AVAILABLE_SDGS); setSearchQuery(''); }}
                ...
                <div className="grid gap-6">
                    {filteredProjects.map((project) => (
                        <div key={project.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-lg transition-all group">
                            <div className="flex flex-col md:flex-row">
                                ...
                                <div className="p-6 md:w-5/12 flex flex-col justify-between border-b md:border-b-0 md:border-r border-gray-100">
                                    <div>
                                        ...
                                        <div className="flex flex-wrap gap-2 mb-4">
                                            {project.maqasid.map(m => <MaqasidIcon key={m} type={m} />)}
                                        </div>
                                        <div className="flex space-x-1">
                                            {project.sdgs.map(s => <SdgBadge key={s} number={s} />)}
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="p-6 md:w-4/12 bg-gray-50 flex flex-col justify-center space-y-4">
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <div className="font-bold text-gray-900 text-lg">$ {project.quickMetrics.needed.toLocaleString()}</div>
                                        </div>
                                        <div>
                                            <div className="text-xs text-gray-500 mb-1">Impact</div>
                                            <div className="font-bold text-gray-900 text-lg">{project.quickMetrics.beneficiaries.toLocaleString()} ppl</div>
                                        </div>
                                ...
                                <div className="p-6 md:w-3/12 flex items-center justify-center bg-white">
                                    <button
                                        className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-4 rounded-xl transition-all shadow-md flex items-center justify-center"
                                    >
                                        Donate Now
                                    </button>
                                </div>
